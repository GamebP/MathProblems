...
so that in fact $|p_n-E(n)|\asymp n\,\ell^3/L^3$ (the omitted Cipolla terms dominate every stretched exponential, since $e^{-c\sqrt L}=o(L^{-A})$ for each fixed $A$). *Audit correction:* de la Vallée Poussin inversion $\pi(x)=\operatorname{li}(x)+O(xe^{-c\sqrt{\ln x}})$ [vK01, DLVP] bounds $p_n-\operatorname{li}^{-1}(n)$, **not** $p_n-E(n)$; no exponential envelope holds against the fixed truncation $E$.
...
$$|\delta_n|\ \asymp\ \frac{|p_n-E(n)|}{n\ln^2 n}\ =\ O\!\Bigl(\frac{(\ln\ln n)^3}{(\ln n)^5}\Bigr)\quad\text{unconditionally (vs.\ the fixed truncation }E\text{)}. \tag{3.1}$$
...
**What RH-grade input would (and would not) give.** Under RH, von Koch [vK01]: $\pi(x)=\operatorname{li}(x)+O(\sqrt x\ln x)$; inverting (mean-value/inverse-function estimate) yields $p_n=\operatorname{li}^{-1}(n)+O(\sqrt n\,\ln^2 n)$, whence the **model residual** $n/p_n-n/\operatorname{li}^{-1}(n)$ satisfies $O(n^{-1/2+o(1)})$; against the fixed truncation $E$, the $(\ln\ln n)^3/(\ln n)^5$ envelope of (3.1) continues to dominate since $n\ell^3/L^3\gg\sqrt n\,\mathrm{polylog}$. `[CONDITIONAL: RH]`
...
We construct a hypothetical "arithmetic world" — a strictly increasing integer sequence $(\tilde p_j)$ playing the role of the primes — exhibiting a persistent odd/even gap bias yet satisfying the prime number theorem through the second-order Cipolla term together with every statistic on a verified finite unconditional list (bounded gaps at positive density, BHP-type upper bounds, sparse large gaps). Components marked (P) are proved below; compatibility items outside this list are labeled [HEURISTIC] and are not claimed as theorems.
...
Every theorem on the verified list above is compatible with these choices; compatibility beyond the list has not been established and is not claimed. Consequently **main-term information alone provably cannot decide EP-15**, and no theorem on the list decides it; whether some different unconditional input could is a metamathematical question on which no theorem is claimed.
...
**EXACT missing implication (quote) — one-way; the converse equivalence is unproved.**
> "Uniform-in-$\lambda$ parity-resolved gap equidistribution $\#\{k\le K:\ g_{2k-1}\le\lambda\,p_{2k-1}/(2k-1)\}=\mu(\lambda)K+o(K)$ across $\lambda$, or a power-saving cancellation bound for $\sum_{n\le x}(-1)^{\pi(n)}$ (the equivalence between these two forms is itself not proved here) $\Rightarrow$ the signed pair series $\sum_k b_k$ converges $\Rightarrow$ $\sum(-1)^nn/p_n$ converges." Tao [Ta23] establishes the needed consequence of this chain modulo the Quantitative HL prime-tuples conjecture (his Conjecture 1.3); nothing weaker is known to supply it — by R5, any input confined to the statistics matched by $W(\theta\neq1,\rho)$ provably cannot suffice.
