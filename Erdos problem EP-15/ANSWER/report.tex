...
\emph{(v)}~An adversarial model world $W(\theta,\rho)$ --- matching PNT main terms through the full second-order Cipolla term and every statistic on a verified finite unconditional list (bounded gaps at positive density, BHP-type upper bounds, sparse large gaps) --- while its paired series diverges shows that main-term statistics alone cannot decide the problem.
\emph{(vi)}~The exact missing implication --- a one-way bridge from uniform parity-resolved gap equidistribution (equivalently, cancellation of $\sum(-1)^{\pi(m)}$) to convergence --- is isolated; it is supplied today only by Hardy--Littlewood strength input.
\end{abstract}
...
Throughout,
\begin{gather*}
a_n:=\frac{n}{p_n},\qquad g_n:=p_{n+1}-p_n,\qquad S_N:=\sum_{n\le N}(-1)^n a_n,\\[6pt]
L_k:=\ln(2k),\qquad \ell_k:=\ln\ln(2k),\qquad \Lambda_k:=L_k+\ell_k .
\end{gather*}
...
=\frac{p_{2k-1}-(2k-1)g_{2k-1}}{p_{2k-1}p_{2k}},\\[4pt]
...
\]
so that in fact $|p_n-E(n)|\asymp n\,\ell^3/L^3$: the omitted Cipolla terms live on scales $n\,\mathrm{poly}(\ell)/L^{j}$ ($j\ge3$), which dominate every stretched exponential since $e^{-c\sqrt L}=o(L^{-A})$ for each fixed $A$. \emph{Audit correction:} de la Vall\'ee Poussin inversion $\pi(x)=\operatorname{li}(x)+O(xe^{-c\sqrt{\ln x}})$ \cite{vK01,DVLP} bounds $p_n-\operatorname{li}^{-1}(n)$, \emph{not} $p_n-E(n)$; no exponential envelope is available against the fixed truncation $E$.
...
|\delta_n|\ \asymp\ \frac{|p_n-E(n)|}{n\ln^2 n}\ =\ O\!\Bigl(\frac{\ell^3}{L^5}\Bigr)
=O\!\Bigl(\frac{(\ln\ln n)^3}{(\ln n)^5}\Bigr)\quad\text{unconditionally (vs.\ the fixed truncation }E\text{)};
\end{equation}
...
Under RH, von Koch \cite{vK01}: $\pi(x)=\operatorname{li}(x)+O(\sqrt{x}\ln x)$; inversion yields $p_n=\operatorname{li}^{-1}(n)+O(\sqrt n\ln^2n)$, whence the \emph{model residual} $n/p_n-n/\operatorname{li}^{-1}(n)$ satisfies $O(n^{-1/2+o(1)})$; against the fixed truncation $E$ the $\ell^3/L^5$ envelope of \eqref{eq:env} continues to dominate, since $n\ell^3/L^3\gg\sqrt n\,\mathrm{polylog}$. But envelope decay alone does not suffice: the comparison sequence $\delta'_n=(-1)^n/(\sqrt n\ln n)$ meets the same envelope while $\sum(-1)^n\delta'_n=\sum1/(\sqrt n\ln n)$ diverges. The Dirichlet-eta phenomenon ($\sum(-1)^nn^{-s+i\tau}$ convergent for $\Re s>0$) relies on rigid monomial structure; interchanging limits over zeros (explicit-formula expansions followed by alternating summation) is not available in any published form. \emph{No published unconditional or RH-only proof exists; the published conditional route is HL-strength (Tao).} Marked honestly: Lemmas~\ref{lem:f}--\ref{lem:delta} and \eqref{eq:env} are theorems; ``RH reopens an eta-type argument'' is plausible-but-unpublished.
...
b_k=\frac{p_{2k-1}-(2k-1)g_{2k-1}}{p_{2k-1}p_{2k}},\\[4pt]
b_k>0\iff g_{2k-1}<\frac{p_{2k-1}}{2k-1}
\end{gathered}}
\end{equation}
...
\end{equation}
so $|b_k|$ has typical size $\asymp1/(k\ln k)$ --- matching the observed $TV(N)\sim0.55\ln\ln N$ --- while its \emph{sign} is governed by whether the odd-indexed normalized gap falls below $1$. Convergence of $\sum_kb_k$ is thus a statement about \emph{parity-resolved statistics of normalized gaps}: persistent odd/even bias creates a one-signed drift $\sim\frac{|1-\theta|}{2}\ln\ln K$. This is the isolated open subproblem; via \eqref{eq:said} it is linked to cancellation in $\sum_m(-1)^{\pi(m)}/(m\ln m)$ --- an equivalence of convergence statements, not of the stronger equidistribution forms.

\section{R5: counterexample world \texorpdfstring{$W(\theta,\rho)$}{W(theta,rho)}}\label{sec:r5}

We construct a hypothetical arithmetic world --- a strictly increasing integer sequence $(\tilde p_j)$ playing the role of primes --- exhibiting persistent odd/even gap bias yet satisfying the prime number theorem through the second-order Cipolla term together with every statistic on a verified finite unconditional list (bounded gaps at positive density, BHP-type upper bounds, sparse large gaps). Items marked (P) are proved here; compatibility items outside this list are marked [HEURISTIC] and are not claimed as theorems.
...
\underbrace{-\frac{1}{2k\Lambda_k^2}}_{\text{systematic }-1\text{ term}},\\[4pt]
...
\[
\begin{gathered}
\sum_{k\le K}\ell_k=\sum_{k\le K}\ln\ln(2k)=K\ln\ln(2K)-\tfrac12\operatorname{li}(2K)+O(\ln\ln K)\\[4pt]
...
\end{itemize}
Every theorem on the verified list above is compatible with these choices. Compatibility with statements outside the list has not been established and is not claimed.

\subsection{Conclusion of conditional independence}

There is a self-consistent world $W(\theta,\rho)$ --- proved to match the primes at PNT scale through the second-order Cipolla term and to satisfy every statistic on the verified finite list --- in which $\sum_k\tilde b_k$ diverges like $\frac{1-\theta}{2}\ln\ln K$. Therefore \textbf{main-term information alone provably cannot decide EP-15, and no theorem on the verified list decides it}; whether some as-yet-unfound unconditional input of different type could decide it is a metamathematical question on which we draw no theorem, though the model is strong evidence that parity-resolved information is indispensable. The missing information is exactly \emph{parity-resolved local gap correlation} --- how $g_{2k-1}$ distributes around $p_{2k-1}/(2k-1)$ versus $g_{2k}$ around $p_{2k}/(2k)$. This is supplied by Hardy--Littlewood: Gallagher's calculation (HL implies Poissonian counts in $(x,x+\lambda\log x]$, mean parity $e^{-2\lambda}$), quantified via Kuperberg's singular-series moment estimates and the Banks--Ford--Tao random sifted model, yielding Tao's bound $\bigl|\sum_{n\le x}(-1)^{\pi(n)}\bigr|\ll x/(\log\log x)^{1.1}$ \cite{Ta23}.
...
\begin{tabular}{@{}rlrr@{}}
\toprule
$N$ & $S_N$ (verbatim fixed-point value) & increment $\Delta_N=S_N-S_{N/2}$ & err.\ bound\\
\midrule
...
\Downarrow\ \text{(one-way; a converse equivalence is not proved)}\\
\text{equidistribution of the parity of }\pi(m)\text{ in intervals }(x,x+\lambda\log x],\\
\text{i.e.}\ \sum_{n\le x}(-1)^{\pi(n)}\ll x/(\log\log x)^{1.1},\\
\Downarrow\\
...
\end{gathered}}
\]
Tao \cite{Ta23} establishes the needed consequence of this chain modulo his Conjecture~1.3 (Quantitative HL prime tuples); nothing weaker is known to supply it --- and by Section~\ref{sec:r5}, any input confined to the statistics matched by $W(\theta\ne1,\rho)$ provably cannot suffice.
