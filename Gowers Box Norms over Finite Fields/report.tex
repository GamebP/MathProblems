...
inequality for arbitrary unimodular data; a complete analysis of the
rigidity property of pointwise rank-one diagonal increments: sufficiency of
the family, sharpness, and an exhaustive small-field refutation showing that
the naive converse classification is false, with the diagonal-cycle twist
identified as the obstruction; and
numerical experiments, including a probe at primes up to $17$, indicating a
degrees-of-freedom-dominated regime at small sizes, where structured and
random diagonal data remain unseparated, together with a decline of the
largest-found cubic-target correlations with $p$ consistent with
$p^{-0.4}$. No
...
\item \emph{Rigidity} (Theorem~\ref{thm:D}, Proposition~\ref{thm:E}). Every
...
structure for most directions. The naive converse classification is false:
an exhaustive small-field analysis exhibits rank-one functions outside the
family and identifies the diagonal-cycle twist as the obstruction.
...
\begin{proposition}[The naive converse classification is false]\label{thm:E}
At $(p,n)=(3,1)$ there exists a unimodular $f:G\times G\to\mathbb{C}$ with
pointwise rank-one diagonal increments that admits \emph{no} representation
$f(x,y)=a(x)b(y)d(x-y)e_p(Q(x,y))$ with unimodular one-variable factors and
quadratic $Q$. Explicitly, writing $\omega=e^{2\pi i/3}$ and enumerating
$G\times G$ by $(x,y)\in\{0,1,2\}^2$, the function
\[
f(x,y)=\begin{cases}
\omega, & (x,y)=(1,2),\\
\omega^{2}, & (x,y)=(2,1),\\
...
\end{cases}
\]
passes the rank-one increment test in every direction: for $h=1$ the
increment matrix carries the additively separable exponent matrix with rows
$(0,1,0)$, $(2,0,2)$, $(0,1,0)$, for $h=2$ the rows $(0,0,2)$, $(0,0,2)$,
$(1,1,0)$ (separable through $A(x)= (0,0,1)$, $B(y)=(0,0,2)$), and $h=0$ is
trivial; whereas an exhaustive search over all $3^{12}$ parameter
assignments $(a,b,d,Q)\in\mathbb{F}_3^{12}$ confirms that no choice
reproduces $f$.
\end{proposition}

\begin{proof}
The increment computation is elementary: for $h=1$ the only deviations of
$f(x+1,y+1)\overline{f(x,y)}$ from unity occur at $(0,1)$, $(1,0)$, $(1,2)$,
$(2,1)$, with respective values $\omega$, $\omega^{2}$, $\omega^{2}$,
$\omega$, giving the displayed separable exponent rows
$\bigl(0,1,0\bigr)$, $\bigl(2,0,2\bigr)$, $\bigl(0,1,0\bigr)$ — additively
separable as $A(x)+B(y)$ with $A=(0,2,0)$, $B=(0,1,0)$; the case $h=2$ is
analogous with rows $(0,0,2)$, $(0,0,2)$, $(1,1,0)$, separable through
$A=(0,0,1)$, $B=(0,0,2)$. Non-representation was certified by exhausting all
$3^{3}$ choices each for the exponent tables of $a$, $b$, $d$ (functions on
$G$) and all $3^{6}$ coefficient tuples for $Q$: for every candidate, $f$
and $a(x)b(y)d(x-y)e_p(Q)$ differ at some point of $G\times G$.
\end{proof}

Anatomy of the failure — and the precise obstruction. The necessity argument
that suggested itself runs as follows: in the coordinates $u=x+y$, $v=x-y$
the rank-one condition is equivalent to the increment law
$\Theta(u+\tau,v)=\Theta(u,v)P_\tau(u+v)N_\tau(u-v)$ for all $u,v,\tau$,
where $\Theta(u,v)=f\bigl(\tfrac{u+v}{2},\tfrac{u-v}{2}\bigr)$; two-step
iteration yields the cocycle relation
$P_\tau(w)P_\sigma(w+\tau)=K(\tau,\sigma)P_\sigma(w)P_\tau(w+\sigma)$ with
the same $K$ for the $N$-family; combining the $(\tau,\sigma)$ and
$(\tau,-\sigma)$ relations produces a multiplicative Jensen equation
$P_\tau(w+\sigma)P_\tau(w-\sigma)=\mu(\tau,\sigma)P_\tau(w)^2$ whose
solutions are characters times quadratic exponentials; single-step
reconstruction from the base $(0,v)$ then suggests
$\Theta(u,v)=\Theta(0,v)\,e_p\bigl(v^{T}M(u)v+\ell(u)\cdot v+\psi(u)\bigr)$,
and a separability argument on the exponent appears to force
$M(u+\tau)=M(u)$ and affine $\ell$. The argument breaks at exactly one
point, and it is instructive: the reconstruction multiplies the
\emph{arbitrary} one-variable function $\Theta(0,v)$ — the prospective
$d(x-y)$ factor — by polynomial-exponential increments, and the concluding
mixed-difference test on the resulting exponent silently assumes that this
factor admits an additive $\mathbb{F}_p$-valued logarithm compatible with
the decomposition. It does not: $\Theta(0,v)$ is an arbitrary unimodular
function, and its freedom absorbs and re-emits mixed content, so the
vanishing of the mixed difference of the \emph{displayed} exponent does not
follow. Concretely, for the counterexample above the increment law
holds in every direction with
$\Theta(0,\cdot)=(1,\omega^{2},\omega)$ and slopes
$\ell(0)=0$, $\ell(1)=\ell(2)=1$ — visibly not affine — while every
instance of the law is nevertheless satisfied by
$P_1=(1,\omega,1)$, $N_1=(1,\omega^{2},1)$ and their analogues: the
non-polynomial factor $d(v)$ soaks up exactly the residual mixed variation.
Exhaustively, the rank-one class at $(3,1)$ has $3^{7}=2187$ elements
against $3^{6}=729$ in the family: the defect is a single additional
$\mathbb{Z}_3$ parameter, visible already as the cycle twist accumulated by
the splitting factors around the wrapped diagonal action. A corrected
classification must therefore enlarge or restructure the target family by
these twist components, and whether the resulting — necessarily
infinite-dimensional — class restores a uniform correlation bound is
precisely the content of Open Problem (O1).
...
A deterministic probe extends the same optimizer to $p\in\{11,13,17\}$: the
alternating phase maximization is identical, retuned only for runtime, run
from three hundred, three hundred, and one hundred twenty random restarts at
$p=11$, $13$, $17$ respectively, with at most two hundred fifty sweeps per
restart and a single fixed deterministic seed. As in Table~\ref{tab:exp},
the reported figures are the largest correlations found by local alternating
maximization — lower bounds for the corresponding suprema, with global
optimality neither claimed nor certified — accompanied by control runs in
which $d$ was taken with uniformly random exponents. Results are collected
in Table~\ref{tab:probe}.
...
prime & target $d$ & statistic & value \\
\midrule
$p=11$ & $e_{11}(s^{3})$ & largest found & $0.542871078$ \\
       &                 & mean          & $0.539892956$ \\
       &                 & sd            & $3.777\times10^{-3}$ \\
$p=13$ & $e_{13}(s^{3})$ & largest found & $0.534867710$ \\
       &                 & mean          & $0.520684259$ \\
       &                 & sd            & $8.575\times10^{-3}$ \\
$p=17$ & $e_{17}(s^{3})$ & largest found & $0.446900156$ \\
       &                 & mean          & $0.443358591$ \\
       &                 & sd            & $3.393\times10^{-3}$ \\
\midrule
$p=11$ & random exponents & largest found correlation & $0.645124308$ \\
$p=13$ & random exponents & largest found correlation & $0.672861647$ \\
$p=17$ & random exponents & largest found correlation & $0.554490869$ \\
\bottomrule
\end{tabular}
\caption{Largest correlations found by local alternating maximization against
unimodular factors at $n=1$: lower bounds for the suprema; a certified global
...
Taken together with the values at $p\le7$, the largest-found cubic-target
correlations $0.724$, $0.677$, $0.543$, $0.535$, $0.447$ at
$p\in\{5,7,11,13,17\}$ decline consistently with a power law $p^{-0.4}$; a
log--log fit across this range gives slope approximately $-0.39$. The
dispersion across restarts is at most $10^{-2}$ for the cubic target, the
optimizer recovering the same basin at every prime, whereas the
random-exponent controls attain larger and less stable values, with standard
deviations reaching $6.473\times10^{-2}$. All of these figures are lower
bounds found by local search, and global optimality is not certified, so the
observed decay constitutes heuristic evidence \emph{against} a positive
uniform lower bound on the maximal-hypothesis correlation for the cubic
diagonal family, not a proof of one. Analytic confirmation remains open;
it would refute the $\delta=1$ case of the problem via
Observation~\ref{obs:C}, since every unimodular diagonal $d(x-y)$ meets the
hypothesis with $\delta=1$.
...
versus $n=1$.
\item[\textnormal{(O2)}] \emph{The converse classification, corrected.} The
naive converse is false (Proposition~\ref{thm:E}); characterize the full
rank-one class, beginning with the role of the diagonal-cycle twist
parameter, and determine whether that class restores a uniform correlation
bound.
