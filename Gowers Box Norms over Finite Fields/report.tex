...
admits two readings. Under the first, $(-1)^{q(x,y)}$ is interpreted as a
quadratic phase $e_p(Q(x,y))$ with
$Q:\mathbb{F}_p^{2n}\to\mathbb{F}_p$ a polynomial of degree at most $2$. Under
the second, $(-1)^{q(x,y)}$ is treated as a genuinely
$\{\pm1\}$-valued factor of unspecified structure. Every result below is
addressed to the larger class
\begin{multline}\label{eq:class}
\bigl\{\, (x,y)\mapsto a(x)\,b(y)\,c(x+y)\,e_p(Q(x,y)) \\
:\; a,b,c:G\to\mathbb{C},\ |a|=|b|=|c|=1,\ Q\ \text{quadratic} \,\bigr\},
\end{multline}
which contains the candidate classes of both readings as subclasses: the
first literally, and the second because a $\{\pm1\}$-valued phase is a
unimodular factor, so that every statement proved for the class
\eqref{eq:class} under hypotheses imposed on $f$ alone applies unchanged to
both readings. Working with the larger class therefore strengthens each
positive structural result and weakens no negative one.
...
structure for most directions. The converse classification is reduced to a
multiplicative parallelogram equation and stated as a conjecture.
...
claimed, and no proof of its full correlation statement is claimed. All
numerical residuals quoted in this paper were independently confirmed to
...
midpoint; carrying out the elimination of the auxiliary splitting factors
leads to the canonical four-point equation below. For every member of the
displayed family whose one-variable constituents are multiplicative phases,
the equation holds immediately, because each constituent sees arguments that
cancel pairwise across the two sides. For completely arbitrary bounded
one-variable constituents the equation imposes genuine additional two-point
identities, and settling exactly which solutions it admits is the content of
the open converse.
...
diagonal increments. Then $\Theta$ satisfies the parallelogram functional
equation
\begin{equation}\label{eq:parallelogram}
\Theta(u+w,\,v+t)\,\Theta(u-w,\,v-t)
\;=\;\Theta(u+t,\,v-w)\,\Theta(u-t,\,v+w).
\end{equation}
Here $u,v,w,t\in G$ are arbitrary. Every solution of \eqref{eq:parallelogram}
is conjectured to belong to the closure of the
family $a(x)b(y)d(x-y)e_p(Q(x,y))$ with $Q$ quadratic. Sufficiency of the
family for pointwise rank-one increments is Theorem~\ref{thm:D}; full
...
hundred sweeps per restart. Results are collected in Table~\ref{tab:exp},
...
\midrule
$p=5$ & $e_5(s^{3})$ & best correlation  & $0.7236067977499792$ \\
$p=5$ & $e_5(s^{3})$ & mean over restarts & $0.7225012249409789$ \\
$p=5$ & $e_5(s^{3})$ & standard deviation & $0.01745$ \\
$p=7$ & $e_7(s^{3})$ & best correlation  & $0.677276973$ \\
$p=7$ & $e_7(s^{3})$ & mean over restarts & $0.675801400$ \\
$p=7$ & $e_7(s^{3})$ & standard deviation & $0.01399$ \\
\midrule
$p=5$ & random exponents & best correlation & $0.7236067977499792$ \\
$p=7$ & random exponents & best correlation & $0.816021079$ \\
\bottomrule
\end{tabular}
\caption{Maximal correlations against unimodular factors at $n=1$.}
\label{tab:exp}
\end{table}
...
through the parallelogram equation \eqref{eq:parallelogram}, force membership
in the family $a(x)b(y)d(x-y)e_p(Q)$.
